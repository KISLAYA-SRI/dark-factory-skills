...
#### Scope Clarification
The embedded prompt says *"SKIP the whole task"* when no Backend API details are provided. **"The whole task" means the BFF task only.** Sitecore and Figma extraction always proceed independently.
...
Prefixes: DN · AC · INT · STATE · SCOPE · PROP · CMS · API *(omit for Presentational)* · COMP · DS · RESP · A11Y · RTL · NFR · FILE · TEST
...
| Phase | Skill | Purpose |
| --- | --- | --- |
| 1 | developer-notes-protocol *(extraction)* | Extract Dev Notes as SACRED LAW |
